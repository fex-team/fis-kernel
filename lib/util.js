...
_.filter = function(str, include, exclude){
    ...
    function match(str, patterns, condition){
        var matched = false;
        if (!_.is(patterns, 'Array')){
            patterns = [patterns];
        }
        patterns.every(function(pattern){
            matched = matched || condition(pattern);
            return !matched;
        });
        return matched;
    }
    ...
    if (include) {
        isInclude = match(str, include, function(reg){
            return str.search(normalize(reg)) < 0;
        });
    }

    if (exclude && !isInclude) {
        isExclude = match(str, exclude, function(reg){
            return str.search(normalize(reg)) > -1;
        });
    }

    return !((!!include && isInclude) || (!!exclude && isExclude));
};
...
_.find = function(rPath, include, exclude){
    ...
    if(path){
        ...
        if(stat.isDirectory() && _.filter(path, include, exclude)){
