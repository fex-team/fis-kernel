...
_.unPack = function(source, dest, res, callback) {
  var packageExt = fis.config.get('system.packageExt', '.tar');
  switch (packageExt) {
    case '.tar':
      var tar = require('tar');
      _.unPack = function (source, dest, res, callback) {
        fs.createReadStream(source)
          ...
          .on('error', function (err) {
            if (callback) {
              ...
              fis.log.error('extract tar file [' + source + '] fail, error [' + err + ']');
            }
          })
          .on('end', function () {
            if (callback && (typeof callback(null, source, res) === 'undefined')) {
              fs.unlinkSync(source);
            }
          });
      };
    ...
    case '.tar.gz':
      ...
      _.unPack = function (source, dest, res, callback) {
        //delay 1s before extract to make sure it works
        setTimeout(function(){
          new targz().extract(source, dest, function (err) {
            if (err) {
              if (callback) {
                ...
                fis.log.error('extract tar.gz file [' + source + '] fail, error [' + err + ']');
              }
            }
            if (callback && (typeof callback(null, source, res) === 'undefined')) {
              fs.unlinkSync(source);
    ...
    case '.zip':
        ...
        fs.createReadStream(source)
          ...
          .on('error', function (err) {
            if (callback) {
              ...
              fis.log.error('extract zip file [' + source + '] fail, error [' + err + ']');
            }
          })
          .on('close', function () {
            if (callback && (typeof callback(null, source, res) === 'undefined')) {
      ...
      break;
    default:
      fis.log.error('no suitable unpack tool');
      return;
  }

  _.unPack(source, dest, res, callback);
          ...
          res
            ...
            .on('end', function(){
              if(status >= 200 && status < 300 || status === 304){
                if(extract){
                    _.unPack(tmp, extract, res, callback);
                } else if(callback && (typeof callback(null, tmp, res) === 'undefined')){
                  fs.unlinkSync(tmp);
                }
              } else if(status >= 300 && status < 400 && res.headers){
                var url = res.headers.location;
                _.download(url, callback, extract);
              }
              else {
...
_.install = function (name, version, opt){
    ...
    var ext = fis.config.get('system.packageExt', '.tar');
    var url = remote + '/' + name + '/' + version + ext;
