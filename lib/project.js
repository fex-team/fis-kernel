...
Object.defineProperty(exports, 'conf', {
    enumerable : true,
    set : function(value){
        FIS_CONF = fis.util.realpathSafe(value);
        if(fis.util.isFile(FIS_CONF)){
            fis.config.merge(fis.util.readJSON(FIS_CONF));
        } else {
            fis.log.warning('fis-conf file [' + value + '] not found.');
        }
    },
    get : function(){
        if(!FIS_CONF){
            fis.log.error('project is uninitialized');
        }
        return FIS_CONF;
    }
});
...
exports.init = function(root, conf, callback){
    ...
    if(fis.util.isDir(root)){
        ...
        if(typeof conf === 'function'){
            callback = conf;
            conf = root + '/' + fis.config.get('system.conf', exports.DEFAULT_CONF_FILE);
        }
