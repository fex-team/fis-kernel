...
function getDomainsByPath(path){
    ...
    if(typeof domain === 'string'){
        ...
        if(typeof domain[0] === 'string') {
            ...
            var matched = domain.some(function(element, index, array) {
                var reg = element.reg, url = element.url, matched;
                if(fis.util.is(reg, 'RegExp')){
                    if(reg.test(path)) {
                        if(typeof url === 'string'){
                            value = url.split(/\s*,\s*/);
                        } else if(fis.util.is(url, 'Array')){
                            value = url;
                        } else {
                            fis.log.warning('invalid domain [' + url + '] of [roadmap.domain.' + index + ']');
                        }
                        return true;
                    }
                } else {
        ...
        fis.util.map(domain, testPath);
    }
    return value;

    function testPath(pattern, domain){
