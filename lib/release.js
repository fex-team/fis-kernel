...
var exports = module.exports = function(opt, callback){
    ...
    //get pack config
    var settings = fis.config.get('pack');
    if(typeof settings === 'undefined'){
        //from fis-pack.json
        var conf = root + '/fis-pack.json';
        if(fis.util.isFile(conf)){
            fis.config.set('pack', fis.util.readJSON(conf));
        }
    }
    
    //package
    if(opt.pack && settings){
        var prepackager = getPackager('prepackager', opt);
        if(prepackager){
            prepackager(ret, settings, opt);
        }
        var packager = getPackager('packager', opt);
        if(packager){
            packager(ret, settings, opt);
        } else {
            var pkgMap = {}, packed = {};
            //construct package table
            fis.util.map(settings, function(path, patterns, index){
        ...
        var postpackager = getPackager('postpackager', opt);
        if(postpackager){
            postpackager(ret, settings, opt);
        }
    }
...
function getPackager(name, opt){
    var packager = fis.config.get('modules.' + name, opt[name]);
    if(packager){
        var type_of = typeof packager;
        if(type_of === 'string'){
            packager = fis.require(name, packager);
        } else if(type_of !== 'function'){
            fis.log.error('invalid packager [' + packager + ']');
        }
    }
    return packager;
}