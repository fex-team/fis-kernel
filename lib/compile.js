...
var exports = module.exports = function(file){
    ...
    if(file.isText()){
        ...
        if(file.isFile()){
            ...
            error('unable to compile [' + file.realpath + ']: Invalid file path.');
        }
...
function extCss(content, callback){
    ...
    callback = callback || function(m, comment, url, filter){
        if(url){
...
function extHtml(content, callback){
    var reg = /(<script(?:\s+[\s\S]*?["'\s\w\/]>|\s*>))([\s\S]*?)(?=<\/script\s*>|$)|(<style(?:\s+[\s\S]*?["'\s\w\/]>|\s*>))([\s\S]*?)(?=<\/style\s*>|$)|<(img|embed|audio|video|link|object)\s+[\s\S]*?["'\s\w\/](?:>|$)|<!--inline\[([^\]]+)\]-->|<!--(?!\[)([\s\S]*?)(-->|$)/ig;
...
function pipe(file, type, ext, keep){
    ...
    fis.util.pipe(key, function(processor, settings, key){
        ...
        try {
            ...
            var msg = key + ': ' + (e.message || e) + ' [' + file.subpath;
            ...
            switch(type){
                ...
                case 'embed':
                    ...
                    if(f && f.isFile()){
                        ...
                        error('unable to embed file[' + value + '] into file[' + path + ']: No such file.');
                    }
                    break;
            }
            return ret;
