...
function refrenceUrl(file, usedUrl) {
    var from = file.release;
    var t = from.substr(1).split('/').length - 1;
    var ret = '';
    for (var i = 0; i < t; i++) {
        ret += '../';
    }
                ...
                switch(type){
                    ...
                    case 'uri':
                        ...
                        if(info.file && info.file.isFile()){
                            ...
                            if (fis.config.get('project.mode') === 'local') {
                                url = info.file.getUrl(exports.settings.hash, false);
                                url = refrenceUrl(file, url);
                            }
