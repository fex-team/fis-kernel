...
function extHtml(content, callback){
    ...
    callback = callback || function(m, $1, $2, $3, $4, $5, $6, $7, $8){
        if($1){//<script>
            ...
            if(tag === 'link'){
                var inline = '';
                //Do not delete the comment of `isNotHtmlImport', it's a feature of html imports for future.
                //var isNotHtmlImport = !/\srel\s*=['"]?import['"]?(?=\s|\/?>)/i.test(m);
